...
EARLIEST_DATE=$(date -u -d "3 years ago" +%Y%m%d)
BACKTESTING_START=$(date -u -d "1 years ago" +%Y%m%d)
...
if [[ ( $# -eq 0 && "$RUN_MODE" == "FreqAI" && "$GITHUB_JOB" == "lexering" ) || ( "$1" == "FreqAI" && "$GITHUB_JOB" != "lexering" ) ]]; then
  pip install -qq --no-cache-dir -r "$FREQAIMODELS_PATH/libs/requirements.txt"
fi
