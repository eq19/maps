...
hyperopt() {
  ...
  jq -c --argjson ids "[$(echo "$*" | sed 's/ /,/g')]" '.pipelines[] | select(.id as $id | $ids | index($id))' $HYPERFILE | while read -r pipeline; do
    ...
    spaces="buy sell roi trailing stoploss protection"
...
freqai() {
  ...
  jq -c --argjson ids "[$(echo "$*" | sed 's/ /,/g')]" '.pipelines[] | select(.id as $id | $ids | index($id))' $HYPERFILE | while read -r pipeline; do
    ...
    if [[ "$GITHUB_JOB" == "lexering" ]]; then
      ...
      jq '.pairlists |= map(if .method == "VolumePairList" then .number_assets = 150 else . end)' $PAIRFILE > tmp.json && mv tmp.json $PAIRFILE
