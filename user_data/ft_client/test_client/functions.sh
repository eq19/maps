...
calculate_score() {
  ...
  local winrate_score=$(echo "$winrate * 20" | bc -l)
  echo "📊 1.1 Winrate: $WINRATE% (score: $(printf "%.2f" "$winrate_score") of 20)"
  ...
  local profit=$(echo "$winrate_score + $profit_mean_score + $profit_total_score" | bc -l)
...
hyperopt() {
  ...
  jq -c --argjson ids "[$(echo "$*" | sed 's/ /,/g')]" '.pipelines[] | select(.id as $id | $ids | index($id))' $HYPERFILE | while read -r pipeline; do
    ...
    jq 'def sort_alpha: if type=="object" then to_entries|sort_by(.key)|from_entries|map_values(sort_alpha) elif type=="array" then map(sort_alpha) else . end; .params |= ({enter,buy,exit,sell,roi,trailing,protection,max_open_trades,stoploss} + del(.enter,.buy,.exit,.sell,.roi,.trailing,.protection,.max_open_trades,.stoploss) | map_values(sort_alpha)) | .params.roi |= with_entries(select(.key|startswith("roi_")|not))' "$STRATEGY" > tmp.$$ && mv tmp.$$ "$STRATEGY"
