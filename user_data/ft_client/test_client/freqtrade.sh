...
cat "$LOG_FILE" | grep --line-buffered -iE "WARNING|ERROR|✅|⏳|📦|🔁|🔄|📎|🧾|ℹ️|🛠️|🔧|💤|⚠️|⛔|❌" | while read -r line; do
...
tail -f "$LOG_FILE" | grep --line-buffered -iE "WARNING|ERROR|✅|⏳|📦|🔁|🔄|📎|🧾|ℹ️|🛠️|🔧|💤|⚠️|⛔|❌" | while read -r line; do
    send_telegram_message "$line"
done
